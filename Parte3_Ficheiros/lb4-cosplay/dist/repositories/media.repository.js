"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MediaRepository = void 0;
const tslib_1 = require("tslib");
const core_1 = require("@loopback/core");
const repository_1 = require("@loopback/repository");
const datasources_1 = require("../datasources");
const models_1 = require("../models");
let MediaRepository = class MediaRepository extends repository_1.DefaultCrudRepository {
    constructor(dataSource, typeRepositoryGetter) {
        super(models_1.Media, dataSource);
        this.typeRepositoryGetter = typeRepositoryGetter;
        this.types = this.createHasManyRepositoryFactoryFor('types', typeRepositoryGetter);
        this.registerInclusionResolver('types', this.types.inclusionResolver);
    }
};
exports.MediaRepository = MediaRepository;
exports.MediaRepository = MediaRepository = tslib_1.__decorate([
    tslib_1.__param(0, (0, core_1.inject)('datasources.db')),
    tslib_1.__param(1, repository_1.repository.getter('TypeRepository')),
    tslib_1.__metadata("design:paramtypes", [datasources_1.DbDataSource, Function])
], MediaRepository);
//# sourceMappingURL=media.repository.js.map